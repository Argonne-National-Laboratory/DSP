...
const char *gDspUsage =
	...
	"Usage: --algo <de,bd,dd,drbd,drdd,dw> [--wassnorm <number> --wasseps <number>] --smps <smps file> [--quad <quad file>] --mps <mps file> --dec <dec file> [--soln <solution file prefix> --param <param file> --test <benchmark objective value>]\n\n"
...
int runDsp(char *algotype, char *smpsfile, char *mpsfile, char *decfile, char *solnfile, char *paramfile, char *testvalue, char *quadfile, double wassparams[2]);
...
#define EXIT_WITH_MSG      \
	...
	else
	{
		...
		// run dsp
		int ret = runDsp(algotype, smpsfile, mpsfile, decfile, solnfile, paramfile, testvalue, quadfile, wassparams);
...
int runDsp(char *algotype, char *smpsfile, char *mpsfile, char *decfile, char *solnfile, char *paramfile, char *testvalue, char *quadfile, double wassparams[2])
{
	...
	{
		if (isroot)
			cout << "Reading SMPS files: " << smpsfile << endl;
	...
	{
		if (isroot)
			cout << "Reading parameter files: " << paramfile << endl;
	...
	{
		if (isroot)
			cout << "Invalid algorithm type, please try again.\n";
		issolved = false;
	}
