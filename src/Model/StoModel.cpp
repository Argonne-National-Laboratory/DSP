	...
	for (int s = 0, r = 0; s < nscen_; ++s)
	{
		if (prob_[s] > 0)
		{
			...
			for (int ss = 0; ss < nscen_; ++ss)
			{
				...
				{
					wass_dist_[r][ss] += pow((*obj_scen_[s])[j] - (*obj_scen_[ss])[j], lp_norm);
					if ((*clbd_scen_[s])[j] > -1.e+20 && (*clbd_scen_[ss])[j] > -1.e+20)
						wass_dist_[r][ss] += pow((*clbd_scen_[s])[j] - (*clbd_scen_[ss])[j], lp_norm);
					if ((*cubd_scen_[s])[j] < 1.e+20 && (*cubd_scen_[ss])[j] < 1.e+20)
						wass_dist_[r][ss] += pow((*cubd_scen_[s])[j] - (*cubd_scen_[ss])[j], lp_norm);
				}
				for (int i = 0; i < nrows_[1]; ++i)
				{
					if ((*rlbd_scen_[s])[i] > -1.e+20 && (*rlbd_scen_[ss])[i] > -1.e+20)
						wass_dist_[r][ss] += pow((*rlbd_scen_[s])[i] - (*rlbd_scen_[ss])[i], lp_norm);
					if ((*rubd_scen_[s])[i] < 1.e+20 && (*rubd_scen_[ss])[i] < 1.e+20)
						wass_dist_[r][ss] += pow((*rubd_scen_[s])[i] - (*rubd_scen_[ss])[i], lp_norm);
					for (int j = 0; j < ncols_[1]; ++j)
					{
						wass_dist_[r][ss] += pow(mat_scen_[s]->getCoefficient(i, j) - mat_scen_[ss]->getCoefficient(i, j), lp_norm);
					}
				}
