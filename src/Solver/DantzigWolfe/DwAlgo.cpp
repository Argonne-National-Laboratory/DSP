...
#define FREE_MEMORY     \
	...
	message_->print(0, "Generated %u initial columns.\n", cols_generated_.size());
...
//		DSPdebug(DspMessage::printArray(ncols_orig_, piA));
		...
		if (phase_ == 2) {
			...
			//DSPdebugMessage("Current lower bound %e\n", curLb);
		...
		double relgap = (si_->getObjValue()-dualobj_)/(1.0e-10+fabs(si_->getObjValue()))*100;
		message_->print(0, "[Phase %d] Iteration %3d: Master objective %e, ", phase_, itercnt, si_->getObjValue());
		if (phase_ == 2)
		...
		/** termination test */
		if (nColsAdded == 0 || terminationTest(itercnt, relgap))
			break;
...
bool DwAlgo::terminationTest(int itercnt, double relgap) {

	bool term = false;
	double feastol = 1.0e-8;

	if (phase_ == 1) {
		if (si_->isProvenOptimal() && si_->getObjValue() < feastol) {
