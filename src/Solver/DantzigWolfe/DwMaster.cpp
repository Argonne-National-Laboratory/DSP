	...
	if (!isInfeasible) {
		...
		if (par_->getBoolParam("DW/APP/ENERGY_STORAGE")) {
			...
			*/
			...
			/** collect equalties for columns*/
			std::vector<std::vector <int>> coupling_col_equalities(master_mat->getNumRows());
			int num_coupling_col_equalities = 0;
			for (int i = 0; i < master_mat->getNumRows(); ++i) {
				...
				for (int j = 0; j < row.getNumElements(); ++j) {
					...
					/** organize equalities for coupling columns, for example, x1=x2=x3 */
					int qindex = -1;
					for (int q = 0; q < num_coupling_col_equalities; ++q) {
						/** check if a column exsits in equality matrix*/
						if (std::find(coupling_col_equalities[q].begin(), coupling_col_equalities[q].end(), colindex) != coupling_col_equalities[q].end()) {
							qindex = q;
							break;
						}
								
					}
					/** add column to equality matrix*/
					if (qindex < 0)	{
						
						/** add new equality*/
						coupling_col_equalities[num_coupling_col_equalities].push_back(row.getIndices()[0]);
                        			coupling_col_equalities[num_coupling_col_equalities].push_back(row.getIndices()[1]);
						//printf("add equality %d with column %6d and column %6d\n", num_coupling_col_equalities, row.getIndices()[0], row.getIndices()[1]);
						num_coupling_col_equalities += 1;
					} else	{
						/** add new column to existing equality */
                        			if (j == 0) {
							if (std::find(coupling_col_equalities[qindex].begin(), coupling_col_equalities[qindex].end(), row.getIndices()[1]) != coupling_col_equalities[qindex].end()) {
                            					//continue;
							} else{
								coupling_col_equalities[qindex].push_back(row.getIndices()[1]);
								//printf("add equality %d with column %6d\n", qindex, row.getIndices()[1]);
							}
                        			} else if (j == 1)  {
                        				if (std::find(coupling_col_equalities[qindex].begin(), coupling_col_equalities[qindex].end(), row.getIndices()[0]) != coupling_col_equalities[qindex].end()) {
								//continue;
							} else{
                            					coupling_col_equalities[qindex].push_back(row.getIndices()[0]);
								//printf("add equality %d with column %6d\n", qindex, row.getIndices()[0]);
							}
                        			}
					}
				...
				if (coupling_solution_index.size() > 0) {
					/** weight average of continuous coupling columns*/
					if (master_block->getCtype()[row.getIndices()[0]] == 'C') {
						double avg_con_cols = 0.0;
						avg_con_cols += weight_con_cols*coupling_solution_value[0];
						avg_con_cols += (1-weight_con_cols)*coupling_solution_value[1];
						coupling_solution_value[0] = avg_con_cols;
						coupling_solution_value[1] = avg_con_cols;
					}
		    ...
		    /** implement heuristic: weight average of time-related columns; keep siting/sizing solutions for all subproblems
			1. go through each subproblems (s)
			2. go through each coupling columns (i)
			3. go through each element (k) of the subproblem and check if the column is in the subproblem
			4. check column type
					if the column is continuous, keep the weight average of equal columns
					if the column is integer, find the row of equalities that has the column, set the value of the column for all equal columns  
			5. store the coupling columns for copy (s) (the number of subproblem)*/ 
 
                    /** store columns generated by heuristics*/
            	    CoinPackedMatrix* coupling_solution_heu = new CoinPackedMatrix(false, 0, 0);
		    for (int s = 0; s < dec->getNumSubproblems(); ++s) {
			std::vector<int> coupling_solution_heu_index;
                    	std::vector<double> coupling_solution_heu_value;
		        for (int i = 0; i < coupling_cols.size(); ++i) {
			     if (master_block->getCtype()[coupling_cols[i]] == 'C') {
				coupling_solution_heu_index.push_back(coupling_cols[i]);
				/** extract the column value*/
				double col_value_temp = 0;
				for (int q = 0; q < coupling_solution->getNumElements(); ++q) {
					if (coupling_solution->getIndices()[q] == coupling_cols[i]){
					    col_value_temp = coupling_solution->getElements()[q];
					    break;
					} 
				}
				coupling_solution_heu_value.push_back(col_value_temp);
			     } else {
			     	for (int k = 0; k < subsols[s]->getNumElements(); ++k) {
				    if (coupling_cols[i] == subsols[s]->getIndices()[k]) {
				        for (int j = 0; j < num_coupling_col_equalities; ++j) {
					    /** find the equalities of the column */
					    if (std::find(coupling_col_equalities[j].begin(), coupling_col_equalities[j].end(), coupling_cols[i]) != coupling_col_equalities[j].end()) {
					        coupling_solution_heu_index.insert(coupling_solution_heu_index.end(),coupling_col_equalities[j].begin(),coupling_col_equalities[j].end());
					        /** extract the column value*/
					        double col_value_temp = 0;
					        for (int q = 0; q < coupling_solution->getNumElements(); ++q) {
						    if (coupling_solution->getIndices()[q] == coupling_cols[i]){
					        ...
					        std::vector<double> col_value_temp_vec(coupling_col_equalities[j].size(), col_value_temp);
					        coupling_solution_heu_value.insert(coupling_solution_heu_value.end(),col_value_temp_vec.begin(),col_value_temp_vec.end());
					        break;
					    }
				        }
				    }
...
void DwMaster::addBranchingCol(const CoinPackedVector& col, double obj) {
	si_->addCol(col, 0.0, COIN_DBL_MAX, obj);
}
