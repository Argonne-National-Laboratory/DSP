	...
	if (!isInfeasible) {
		...
		if (par_->getBoolParam("DW/APP/ENERGY_STORAGE")) {
			...
			*/
			...
			std::vector<std::vector <int>> coupling_col_equalities;
			...
			for (int i = 0; i < master_mat->getNumRows(); ++i) {
				...
				if (coupling_solution_index.size() > 0) {
                    ...
                    /** implement heuristic: average of siting/sizing for all subproblems and time-related variables
                     another way: keep siting/sizing for all subproblems, should record subproblem number for columns; weight average of time-related columns
                     set weight parameter for time-related variables in coupling constraints
                     float weight_time_related = 0.5;
                     */
                    std::vector<int> coupling_solution_heu_index;
                    std::vector<double> coupling_solution_heu_value;
                    
                    for (int i = 0; i < num_coupling_col_equalities; ++i) {
                        double sum_temp = 0.0;
                        for (int j = 0; j < coupling_col_equalities[i].size(); ++j) {
                            /** find the coupling column index of equalities*/
                            std::vector<int>::iterator index_temp;
                            index_temp = std::find(coupling_solution_index.begin(), coupling_solution_index.end(), coupling_col_equalities[i][j]);
                            if (index_temp != coupling_solution_index.end()) {
                                sum_temp += coupling_solution_value[index_temp - coupling_solution_index.begin()];
                            }
                        }
                        /** average for all equal coupling columns */
                        sum_temp = sum_temp/double(coupling_col_equalities[i].size());
                        /** store columns generated by heuristics */
                        for (int j = 0; j < coupling_col_equalities[i].size(); ++j) {
                            coupling_solution_heu_index.push_back(coupling_col_equalities[i][j]);
                            coupling_solution_heu_index.push_back(sum_temp);
                        }
