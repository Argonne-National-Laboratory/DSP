...
#define FREE_MEMORY    \
	...
	DSP_RTN_CHECK_THROW(findLowerBound(lowerbound), "findLowerBound", "TssBd");
	...
	DSP_RTN_CHECK_THROW(constructMasterProblem(tssbdsub, lowerbound), "constructMasterProblem", "TssBd");
	...
	{
		/** configure LP */
		DSP_RTN_CHECK_THROW(configureSLP(), "configureSLP", "TssBd");

		/** solve LP */
		DSP_RTN_CHECK_THROW(solveSLP(tssbdsub), "solveSLP", "TssBd");
	}
	else
	{
		/** configure MILP */
		DSP_RTN_CHECK_THROW(configureSMILP(tssbdsub), "configureSMILP", "TssBd");
...
/** construct master problem */
DSP_RTN_CODE TssBd::constructMasterProblem(TssBdSub * tssbdsub, double lowerbound)
{
#define FREE_MEMORY       \
	FREE_PTR(mat)         \
	FREE_ARRAY_PTR(clbd)  \
...
/** configure Phase 1 */
DSP_RTN_CODE TssBd::configureSLP()
{
	BGN_TRY_CATCH
...
DSP_RTN_CODE TssBd::findLowerBound(double & lowerbound)
{
#define FREE_MEMORY       \
	FREE_PTR(mat)         \
	FREE_ARRAY_PTR(clbd)  \
...
#pragma omp parallel for schedule(dynamic)
	...
	END_TRY_CATCH_RTN(FREE_MEMORY;FREE_2D_PTR(model_->getNumScenarios(), si);,DSP_RTN_ERR)
 ...
 */
DSP_RTN_CODE TssBd::solveSLP(TssBdSub * tssbdsub)
{
//#define CUT_AGGREGATION
	...
	{
		/** solution status */
		if (si_->getStatus() == DSP_STAT_OPTIMAL)
		{
...
DSP_RTN_CODE TssBd::configureSMILP(TssBdSub * tssbdsub)
{
	BGN_TRY_CATCH
