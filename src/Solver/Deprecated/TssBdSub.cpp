...
/** load problem */
DSP_RTN_CODE TssBdSub::loadProblem(
		TssModel * model,
		int naugs,
		int * augs,
		int nAuxvars)
...
#define FREE_MEMORY            \
	...
	for (unsigned int i = 0; i < scenarios_.size(); ++i)
	{
		...
		DSP_RTN_CHECK_THROW(model->copyRecoProb(s, mat_mp_[s],
...
#pragma omp parallel for schedule(dynamic)
	...
	for (int s = nSubs_ - 1; s >= 0; --s)
	{
		...
		else if (status_[s] == DSP_STAT_DUAL_INFEASIBLE)
			status = status_[s];
		if (status_[s] == DSP_STAT_ABORT ||
			status_[s] == DSP_STAT_UNKNOWN)
		{
			status = status_[s];
			break;
		}
...
#define FREE_MEMORY                       \
	...
	for (s = nSubs_ - 1; s >= 0; --s)
	{
		...
		if (status_[s] != DSP_STAT_OPTIMAL) continue;
