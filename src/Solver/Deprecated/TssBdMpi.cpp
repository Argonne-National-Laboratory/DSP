...
/** initialize solver */
DSP_RTN_CODE TssBdMpi::initialize()
{
#define FREE_MEMORY \
	...
	{
		int effective_comm_size = CoinMin(comm_size_, model_->getNumScenarios());
		if (comm_rank_ < effective_comm_size)
		{
			/** round-robin */
			int size = (int) ((model_->getNumScenarios() - 1) - comm_rank_) / effective_comm_size + 1;
			assert(size > 0);
			parProcIdxSize_ = size;
			parProcIdx_ = new int [size];
			int i = 0;
			for (int s = comm_rank_; s < model_->getNumScenarios(); s += effective_comm_size)
				parProcIdx_[i++] = s;
		}
	}
	...
	if (comm_rank_ == 0)
	{
		probability_ = new double [model_->getNumScenarios()];
...
#define FREE_MEMORY    \
	...
	DSP_RTN_CHECK_THROW(tssbdsub->loadProblem(model_, naugs_, augs_, naux_), "loadProblem", "TssBdSub");
	...
	DSP_RTN_CHECK_THROW(findLowerBound(lowerbound), "findLowerBound", "TssBdMpi");
	...
	DSP_RTN_CHECK_THROW(constructMasterProblem(tssbdsub, lowerbound), "constructMasterProblem", "TssBdMpi");
	message_->print(1, " (time elapsed: %.2f sec)\n", CoinGetTimeOfDay() - swtime);
	...
	DSP_RTN_CHECK_THROW(configureMaster(tssbdsub), "configureMaster", "TssBdMpi");

	/** run workers */
	DSP_RTN_CHECK_THROW(runWorkers(tssbdsub), "runWorkers", "TssBdMpi");

	/** solve Benders master */
	DSP_RTN_CHECK_THROW(runMaster(tssbdsub), "runMaster", "TssBdMpi");
...
DSP_RTN_CODE TssBdMpi::findLowerBound(double & lowerbound)
{
#define FREE_MEMORY       \
	FREE_PTR(mat)         \
	FREE_ARRAY_PTR(clbd)  \
...
/** construct master problem */
DSP_RTN_CODE TssBdMpi::constructMasterProblem(TssBdSub * tssbdsub, double lowerbound)
{
#define FREE_MEMORY       \
	FREE_PTR(mat)         \
	FREE_ARRAY_PTR(clbd)  \
...
/** configure master */
DSP_RTN_CODE TssBdMpi::configureMaster(TssBdSub * tssbdsub)
{
	if (comm_rank_ != 0)
		return DSP_RTN_OK;
	...
	{
		/** configure LP */
		DSP_RTN_CHECK_THROW(configureSLP(), "configureSLP", "TssBdMpi");
	}
	else
	{
		/** configure MILP */
		DSP_RTN_CHECK_THROW(configureSMILP(tssbdsub), "configureSMILP", "TssBdMpi");
	}
...
/** solve Benders master */
DSP_RTN_CODE TssBdMpi::runMaster(TssBdSub * tssbdsub)
{
	if (comm_rank_ != 0)
		return DSP_RTN_OK;
	...
	{
		/** solve Stochastic LP */
		DSP_RTN_CHECK_THROW(solveSLP(tssbdsub, si_), "solveSLP", "TssBd");
	}
	else
	{
		/** solve Stochastic LP */
		...
		DSP_RTN_CHECK_THROW(solveSMILP(), "solveSMILP", "TssBd");
	}
...
/** run workers */
DSP_RTN_CODE TssBdMpi::runWorkers(TssBdSub * tssbdsub)
{
#define FREE_MEMORY \
...
/** configure Phase 1 */
DSP_RTN_CODE TssBdMpi::configureSLP()
{
	BGN_TRY_CATCH
...
DSP_RTN_CODE TssBdMpi::configureSMILP(TssBdSub * tssbdsub)
{
	BGN_TRY_CATCH
 ...
 */
DSP_RTN_CODE TssBdMpi::solveSLP(
		TssBdSub * tssbdsub,
		SolverInterface * si,
		int whereFrom,
		OsiCuts * outcuts)
{
	...
	{
		/** solution status */
		if (si->getStatus() == DSP_STAT_OPTIMAL)
		{
...
DSP_RTN_CODE TssBdMpi::solveSMILP()
{
	BGN_TRY_CATCH
