...
/** initialize solver */
DSP_RTN_CODE DecDdMpi::initializeGlobal()
{
	BGN_TRY_CATCH
	...
	/** set scenario indices for each process */
	if (parProcIdxSize_ > 0)
	{
		parProcIdx_ = new int [parProcIdxSize_];
		CoinCopyN(par_->getIntPtrParam("ARR_PROC_IDX"), parProcIdxSize_, parProcIdx_);
	}
	else
	{
		int effective_comm_rank = comm_rank_;
		int effective_comm_size = comm_size_;
		if (use_root_process_ == false)
			effective_comm_size--;
...
DSP_RTN_CODE DecDdMpi::initializeLocal()
{
	BGN_TRY_CATCH
	...
	if (!model_->nonanticipativity() || !model_->isStochastic())
	{
		if (parFeasCuts_ >= 0 || parOptCuts_ >= 0)
			message_->print(0, "Warning: Feasibility and optimality cuts are not supported in a general decomposition model.\n");
		if (parEvalUb_ >= 0)
			message_->print(0, "Warning: Primal heuristic is not supported in a general decomposition model.\n");
		parFeasCuts_ = -1;
		parOptCuts_ = -1;
		parEvalUb_ = -1;
	}
	...
	if (parNumCutsPerIter_ > model_->getNumSubproblems() || parNumCutsPerIter_ < 1)
		parNumCutsPerIter_ = model_->getNumSubproblems();
...
/** solve */
DSP_RTN_CODE DecDdMpi::solve()
{
	BGN_TRY_CATCH

	if (use_root_process_ == false && comm_size_ < 2)
	{
		message_->print(0, "Error: At least two processors are required.\n");
		status_ = DSP_STAT_STOPPED_MPI;
		return DSP_RTN_ERR;
	}
	...
	/** configure additional settings for subproblem solver */
	configureSubSolver();
	...
	message_->print(1,
			"Time elapsed for creating master and subproblems: %.2f seconds\n",
...
DSP_RTN_CODE DecDdMpi::initializeStatistics()
{
	BGN_TRY_CATCH
...
/** create subproblem */
DSP_RTN_CODE DecDdMpi::createSubproblem()
{
#define FREE_MEMORY \
	...
	if (use_root_process_ == true || comm_rank_ != 0)
	{
...
DSP_RTN_CODE DecDdMpi::configureSubSolver()
{
	BGN_TRY_CATCH
	...
	if (parFeasCuts_ >= 0 || parOptCuts_ >= 0 || parEvalUb_ >= 0)
	{
		...
		if (tssModel)
		{
			...
			{
				/** add cut generator */
				subprobs_[s]->addCutGenerator(bdsub_);
			}
		}
		else
		{
			message_->print(0, "Error: Cuts and primal heuristic are not supported in a general decomposition model\n");
			return DSP_RTN_ERR;
		}
	}
...
/** solve subproblem */
DSP_RTN_CODE DecDdMpi::solveSubproblem()
{
	int doContinue = 1;
	Solutions solutions;
...
#define FREE_MEMORY \
	...
	stime = MPI::Wtime();
	if (comm_group_ == 0 && (comm_rank_ != 0 || use_root_process_ == true))
	{
		int offset = 0;
...
/** create master problem */
DSP_RTN_CODE DecDdMpi::createMaster()
{
	if (comm_rank_ != 0)
		return DSP_RTN_OK;
...
DSP_RTN_CODE DecDdMpi::solveMaster()
{
	if (comm_rank_ != 0)
		return DSP_RTN_OK;
	...
	if (parLogDualVars_)
	{
		...
		{
			double mult = master_->getLagrangian()[i] - lambda_[i];
			dist += mult * mult;
		}
		dist = sqrt(dist);
		...
		/** update multipliers */
		CoinCopyN(master_->getLagrangian(), nmult, lambda_);
	}
...
DSP_RTN_CODE DecDdMpi::syncMaster()
{
#define FREE_MEMORY \
	FREE_ARRAY_PTR(sendbuf); \
	FREE_ARRAY_PTR(recvbuf); \
	...
	if (comm_group_ == 0 && (use_root_process_ == true || comm_rank_ != 0))
	{
...
bool DecDdMpi::doTerminate()
{
	...
	if (comm_rank_ == 0)
	{
		...
		message_->print(1, "Iteration %3d: Dual Bound %e", iterCnt_, dualBound_);
		if (parEvalUb_ >= 0)
		{
			message_->print(1, ", Primal Bound %e ", primalBound_);
			if (gapPrimal > 1.e+4 || gapPrimal < 1.e-4)
				message_->print(1, "(Optimality Gap %.2e %%)", gapPrimal * 100);
			else
				message_->print(1, "(Optimality Gap %.2f %%)", gapPrimal * 100);
		}
		message_->print(1, ", Time Elapsed %.2f sec.", MPI::Wtime() - wtime_start_);
		if (parMasterAlgo_ != Subgradient && parLogLevel_ > 1)
		{
			...
			if (parLogLevel_ > 2)
			{
				if (gapMaster > 1.e+4 || gapMaster < 1.e-4)
					message_->print(2, " (Gap %.2e %%)", gapMaster * 100);
				else
					message_->print(2, " (Gap %.2f %%)", gapMaster * 100);
			}
			message_->print(1, ", Solution Time %.2f sec.", wtime_master_[wtime_master_.size() - 1]);
		}
		message_->print(1, "\n");
		...
		else if (status_ != DSP_STAT_OPTIMAL)
			terminate = true;
...
/** collect results */
DSP_RTN_CODE DecDdMpi::collectResults()
{
		...
		bool & feasible          /**< indicating feasibility */)
{
	...
	for (int s = 0; s < nsubprobs; ++s)
	{
		...
		/** infeasible ?*/
		if (bdsub_->status_[subprobs_[s]->sind_] == DSP_STAT_PRIM_INFEASIBLE)
		{
			feasible = false;
			break;
		}
		...
		int & numSyncedSolutions)
{
	...
	MPIAllgatherCoinPackedVectors(comm_, solutionsToSync, solutionsGathered);
		...
		OsiCuts * cuts /**< cuts to synchronize */)
{
	...
	/** synchronize cuts */
	MPIAllgatherOsiCuts(comm_, cutsToSync, cutsGathered);
...
double DecDdMpi::ticToc()
{
	time_remains_ -= MPI::Wtime() - tic_;
	tic_ = MPI::Wtime();
	return time_remains_;
}
